...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the full offline indexing workflow: OCR graph -> node embeddings -> CSE-enriched graph."
    )
    parser.add_argument(
        "image",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output directory. Defaults to artifacts/node_stores/<image_stem>/.",
    )
    ...
    parser.add_argument("--qwen-temperature", type=float, default=0.1, help="Temperature for Qwen region analysis.")
...
def main() -> None:
    ...
    qwen_crops_dir = resolve_optional_output_dir(args.qwen_crops_dir)
    ...
    pipeline = run_document_pipeline(
        ...
        qwen_labels=[label.strip() for label in args.layout_labels.split(",") if label.strip()],
    ...
    if args.output_dir is not None:
        ...
        output_dir = embed_config.output_root / image_path.stem
