...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run top-k seed retrieval -> per-seed CSE -> top-k subgraph QA with Qwen2.5-VL."
    )
    parser.add_argument(
        "store_dir",
        type=Path,
        help="Directory containing graph_enriched.json, embeddings.npy, and embedding_meta.json.",
    )
    parser.add_argument(
        "query",
        help="User query used to retrieve seed nodes, expand subgraphs, and answer.",
    )
    ...
    parser.add_argument(
        ...
        help="Qwen2.5-VL model used for final answering.",
    )
    ...
    parser.add_argument("--max-images", type=int, default=4, help="Maximum number of region crops passed into Qwen for each expanded subgraph.")
    parser.add_argument(
        "--subgraph-output",
        type=Path,
        default=None,
        help="Optional path to save the reranked top-k subgraphs JSON payload.",
    )
    parser.add_argument(
        "--answer-output",
        type=Path,
        default=None,
        help="Optional path to save full answer JSON.",
    )
...
def main() -> None:
    ...
    seed_node_types = tuple(
        item.strip().lower() for item in args.seed_node_types.split(",") if item.strip()
    )
    allowed_relations = tuple(
        item.strip() for item in args.allowed_relations.split(",") if item.strip()
    ) or None
